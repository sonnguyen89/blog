...
if (!function_exists( 'woo_custom_styling')) {
	function woo_custom_styling() {
		...
		if ($link) 
			$output .= 'a:link, a:visited, ul.products li a.button {color:'.$link.'} ul.products li a.button {color:'.$link.'!important}' . "\n";

		if ($hover)
			$output .= 'a:hover, .post-more a:hover, .post-meta a:hover, .post p.tags a:hover, ul.products li a.button:hover {color:'.$hover.'} ul.products li a.button:hover {color:'.$hover.'!important}' . "\n";

		if ($button) {
			$output .= 'a.button, .button.alt, .button, input.button, button.button, a.comment-reply-link, #commentform #submit, #featured-products ul.products .button {background:'.$button.';border-color:'.$button.'}' . "\n";
			$output .= 'a.button:hover, .button.alt:hover, .button:hover, input.button:hover, button.button:hover, a.comment-reply-link:hover, #commentform #submit:hover, #featured-products ul.products .button:hover {background:'.$button.';opacity:0.8;}' . "\n";
		}
...
if (!function_exists( 'woo_custom_typography')) {
	function woo_custom_typography() {
		...
		// Add default typography Google Font
			...
			// $woo_options['woo_default_face'] = array('face' => 'PT Sans');
...
add_action('wp_head', 'woo_jcarousel_script');

function woo_jcarousel_script(){
	...
	<script type="text/javascript">
		...
		jQuery(document).ready(function() {
		    jQuery('#featured-products.fp-slider ul.featured-products').jcarousel({scroll: 2});
		});
	</script>
